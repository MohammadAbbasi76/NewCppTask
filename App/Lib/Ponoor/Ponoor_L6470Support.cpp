...
int32_t AutoDriver::paramHandler(uint8_t param, uint32_t value) {
  ...
  //  bits and do the right number of transfers. That is handled by the
  //  xferParam() function, in most cases, but for 1-uint8_t or smaller
  //  transfers, we call SPIXfer() directly.
  switch (param) {
  // ABS_POS is the current absolute offset from home. It is a 22 bit number
  // expressed
  //  in two's complement. At power up, this value is 0. It cannot be written
  //  when the motor is running, but at any other time, it can be updated to
  //  change the interpreted position of the motor.
  case ABS_POS:
    retVal = xferParam(value, 22);
    break;
  // EL_POS is the current electrical position in the step generation cycle. It
  // can
  //  be set when the motor is not in motion. Value is 0 on power up.
  case EL_POS:
    retVal = xferParam(value, 9);
    break;
  // MARK is a second position other than 0 that the motor can be told to go to.
  // As
  ...
  //  to get infinite acceleration/decelaeration- there is no way to get
  //  infinite deceleration w/o infinite acceleration (except the HARD STOP
  ...
  //  feature and the lowest speed the motor will be allowed to operate at.
  //  LSPD_OPT is the 13th bit, and when it is set, the minimum allowed speed is
  //  automatically set to zero. This value is 0 on startup.
  // MinSpdCalc() function exists to convert steps/s value into a 12-bit value
  // for this
  //  register. SetLSPDOpt() function exists to enable/disable the optimization
  //  feature.
  case MIN_SPEED:
  ...
  // FSCalc() function exists to convert steps/s value into 10-bit integer for
  // this
  //  register.
  case FS_SPD:
    retVal = xferParam(value, 10);
  ...
  // ratiometric
  //  representations: 255 for full output voltage, 128 for half, etc. Default
  //  is 0x29.
  // The implications of different KVAL settings is too complex to dig into
  // here, but
  //  it will usually work to max the value for RUN, ACC, and DEC. Maxing the
  //  value for HOLD may result in excessive power dissipation when the motor is
  //  not running.
  case KVAL_HOLD:
  ...
  // datasheet
  //  for full details on operation- the default value should be okay for most
  //  users.
  case K_THERM:
    value &= 0x0F;
    retVal = xferParam(value, 8);
  ...
  // measurements.
  //  This is less useful than it sounds; see the datasheet for more
  //  information.
  case ADC_OUT:
    retVal = xferParam(value, 8);
    break;
  ...
  case OCD_TH:
    ...
    retVal = xferParam(value, 8);
    break;
  // Stall current threshold. Defaults to 0x40, or 2.03A. Value is from 31.25mA
  // to
  ...
  case STALL_TH:
    ...
    retVal = xferParam(value, 8);
    break;
  // STEP_MODE controls the microstepping settings, as well as the generation of
  // an
  //  output signal from the dSPIN. Bits 2:0 control the number of microsteps
  //  per step the part will generate. Bit 7 controls whether the BUSY/SYNC pin
  //  outputs a BUSY signal or a step synchronization signal. Bits 6:4 control
  //  the frequency of the output signal relative to the full-step frequency;
  //  see datasheet for that relationship as it is too complex to reproduce
  //  here.
  // Most likely, only the microsteps per step value will be needed; there is a
  // set
  ...
  // constants
  //  is provided to make this easy to interpret. By default, ALL alarms will
  //  trigger the FLAG pin.
  case ALARM_EN:
    retVal = xferParam(value, 8);
    break;
  // CONFIG contains some assorted configuration bits and fields. A fairly
  // comprehensive
  //  set of reasonably self-explanatory constants is provided, but users should
  //  refer to the datasheet before modifying the contents of this register to
  //  be certain they understand the implications of their modifications. Value
  //  on boot is 0x2E88; this can be a useful way to verify proper start up and
  //  operation of the dSPIN chip.
  case CONFIG:
    retVal = xferParam(value, 16);
    break;
  // STATUS contains read-only information about the current condition of the
  // chip. A
  //  comprehensive set of constants for masking and testing this register is
  //  provided, but users should refer to the datasheet to ensure that they
  //  fully understand each one of the bits in the register.
