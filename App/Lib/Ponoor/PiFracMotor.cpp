...
PiFracMotor::~PiFracMotor() {
  // we are done communicating with this particular motor, so put its chip
  // select high
  setCSPin(true);
}
...
/// <param name="csPin">Chip select pin. Pin on the micro that is used to select
/// the motor controller chip. Set to low in order to enable the motor
/// controller chip.</param> <returns>true if the motor driver chip is already
/// executing a particular move.</returns>
...
/// <summary>
/// Configure the maximum and full (steady state) speed of the motor in steps
/// per second.
/// </summary>
/// <param name="csPin">Chip select pin. Pin on the micro that is used to select
/// the motor controller chip. Set to low in order to enable the motor
...
/// <param name="csPin">Chip select pin. Pin on the micro that is used to select
/// the motor controller chip. Set to low in order to enable the motor
/// controller chip.</param> <param name="stepMode">number between STEP_FS and
/// STEP_FS_128 that corresponds to the microstepping setting of the
/// motor.</param>
...
void PiFracMotor::configureDefault(int csPin, ConcreteSPI *SPI) {
  ...
  motorDriver->setSlewRate(
      SR_530V_us); // Upping the edge speed increases torque.
  ...
  motorDriver->setHoldKVAL(
      32); // This controls the holding current; keep it low.
}

/// <summary>
/// Set CS pin high or low
/// </summary>
/// <param name="csPinHigh">set to true to set CS pin high, or false to set it
/// low.</param>
void PiFracMotor::setCSPin(bool csPinHigh) {
  if (csPinHigh) {
    gpio.digitalWrite(this->csPin, HIGH);
  } else {
    gpio.digitalWrite(this->csPin, LOW);
  }
  // make sure pin is at low or high state for at least 1 us before doing
  // anything
